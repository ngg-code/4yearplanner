...
('POL',220,'POL 220','American Politics',4),
('POL',221,'POL 221','American Politics II',4),
...
('POL',354,'POL 354','Political Economy of Development',4),
...
('PST',220,'PST 220','Applied Policy Analysis (cross-listed)',4),
('PST',320,'PST 320','Applied Policy Analysis (cross-listed 300-level)',4)
...
SELECT m.id,
       ...
       'choose_one',
       1,
       'Choose one course from the American Politics subfield: POL 216, POL 219, POL 237, POL 239, or POL 295 as approved.',
       20
FROM majors m
WHERE m.code = 'POL'
ON CONFLICT (major_id, code) DO NOTHING;
...
SELECT m.id,
       ...
       'choose_one',
       1,
       'Choose one course from the Comparative Politics subfield: POL 255, POL 257, POL 258, POL 262, POL 273, or POL 295 as approved.',
       30
FROM majors m
WHERE m.code = 'POL'
ON CONFLICT (major_id, code) DO NOTHING;
...
SELECT m.id,
       ...
       'choose_one',
       1,
       'Choose one course from the International Politics subfield: POL 250, POL 251, POL 259, or POL 295 as approved.',
       40
FROM majors m
WHERE m.code = 'POL'
ON CONFLICT (major_id, code) DO NOTHING;
...
SELECT m.id,
       'POL_STATS',
       'Statistics requirement: SST 115 or STA 209 (not counted in 32 credits)',
       'choose_one',
       1,
       'Students must take SST 115 or STA 209 at Grinnell. AP Statistics and transfer credits do not apply. This requirement is in addition to the 32 major credits.',
       60
FROM majors m
WHERE m.code = 'POL'
ON CONFLICT (major_id, code) DO NOTHING;
...
SELECT m.id,
       ...
       'custom',
       32,
       'Minimum 32 credits total, including POL 101, one course in each of three subfields (American, Comparative, International), and 8 credits at the 300-level. Statistics (SST 115 or STA 209) is required in addition to the 32 credits. Up to 8 credits may be taken outside the department with advisor approval.',
       80
FROM majors m
WHERE m.code = 'POL'
ON CONFLICT (major_id, code) DO NOTHING;
...
JOIN courses c ON c.course_code IN (
  'POL 216','POL 219','POL 237','POL 239','POL 295'
)
WHERE m.code = 'POL'
...
JOIN majors m ON m.id = b.major_id
JOIN courses c ON c.course_code IN (
  'POL 255','POL 257','POL 258','POL 262','POL 273','POL 295'
)
WHERE m.code = 'POL'
...
JOIN majors m ON m.id = b.major_id
JOIN courses c ON c.course_code IN (
  'POL 250','POL 251','POL 259','POL 295'
)
WHERE m.code = 'POL'
...
JOIN majors m ON m.id = b.major_id
JOIN courses c ON c.course_code IN (
  'POL 310','POL 319','POL 320','POL 325','POL 335',
  'POL 352','POL 354','POL 355','POL 356','POL 357',
  'PST 320'
)
WHERE m.code = 'POL'
